...
import { emptyMentionProvider } from '../mentionProvider';
import type { WorkflowPromptContent } from '../promptContent';
...
export type { CanvasMentionProvider, WorkflowMentionItem } from './types';
export type { WorkflowPromptContent } from '../promptContent';

export type GenerationPromptEditorProps = {
...
export type CanvasPromptEditorProps = GenerationPromptEditorProps;
...
export function GenerationPromptEditor({
  ...
  enableMention = true,
  mentionProvider = emptyMentionProvider,
  onChange,
  onEnterSubmit,
  className,
  readOnly = false,
  ...
  const editor = useEditor(
    {
      ...
      editorProps: {
        ...
        handleKeyDown: (_view, event) => {
          if (event.key === 'Enter' && !event.shiftKey && onEnterSubmitRef.current) {
            event.preventDefault();
...
/** 兼容画布旧 import 名 */
export const CanvasPromptEditor = GenerationPromptEditor;
